...
- **All tests must pass before any work is considered done.** Run `make check` (lint + format check + test) and confirm zero failures, zero warnings.
- **Format with shfmt** before committing: `make fmt`. Settings: 2-space indent, binary ops on next line, case body indented.
...
3. **Verify**: `make check` — zero shellcheck findings, zero formatting diffs, zero test failures
